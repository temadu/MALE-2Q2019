...
def logisticRegressionInclusive(trainSet = [], testSet = []):
  ...
  xs = np.array(xs)
  y = np.array(y)
  model = linear_model.LogisticRegression(solver='lbfgs', random_state=0).fit(xs, y)
  r_sq = model.score(xs, y)
...
def logisticRegressionExclusive(trainSet=[], testSet=[]):
  ...
  y = []
  for i in range(len(trainSet[0])):
    xs.append([trainSet[0][i], trainSet[1][i], trainSet[2][i], trainSet[3][i]])
    y.append(trainSet[4][i])
  xs = np.array(xs)
  y = np.array(y)
  model = linear_model.LogisticRegression(solver='lbfgs', random_state=0).fit(xs, y)
  ...
  y2 = []
  for i in range(len(testSet[0])):
    x2s.append([testSet[0][i], testSet[1][i], testSet[2][i], testSet[3][i]])
    y2.append(testSet[4][i])
  x2s = np.array(x2s)
  y2 = np.array(y2)
  predictions = model.predict(x2s)
...
if __name__ == "__main__":
  ...
  dataset = loadDataset('data/acath.csv')
